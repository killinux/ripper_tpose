...
部分角色只保留了这些顶点组的名字，眼球顶点却没有足够权重。例如 a08 的
`pc_a_nk_eyes` 包含 864 个眼球面，但旧算法会因为场景中存在 `Eyeball` 组而关闭
几何兜底，把它们全部判成脸。v1.1.3 起会组合“骨骼权重 + 连通块/UV 几何 +
原始 `eye/eyes/iris` 材质名”判断；独立的「修复眼睛」按钮也使用同一规则。

b02 还有另一种混合情况：`pc_b_nk_eyebrow` 同时包含眉毛、上睫毛和下睫毛，
而下睫毛的 Eyelid 权重不是主权重；`pc_b_nk_tears` 则由许多很小的透明眼部卡片组成。
v1.1.5 起先保留可明确判断的 Eyebrow/Eyelid 骨骼结果，只对剩余
`brow/eyebrow/lash` 连通块按相对眼球高度兜底，并把原始 `tear/tears` 材质统一映射
为透明罩层。v1.1.6 进一步覆盖非 head 网格中的同名 tear 材质槽；b02 的身体网格也
带有 32 个这类面。这样不会把下睫毛染成肤色，也不会在单侧眼睛上留下像眼镜片的实体块。
