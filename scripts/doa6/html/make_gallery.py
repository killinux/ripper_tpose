...
{cards}  </div>
  ...
  <section class="appendix">
    <h2>附录 · 导出脚本用法</h2>
    <p>脚本在 <code>scripts\\doa6\\</code>，需在装有游戏的机器上运行。
      完整说明见同目录 <code>EXPORT_GUIDE.md</code>（操作）与 <code>README.md</code>（格式与原理）。</p>

    <h3>角色由三个部件组成</h3>
    <table>
      <tr><th>部件</th><th>条目名</th><th>来源</th></tr>
      <tr><td>服装 + 身体</td><td><code>&lt;角色&gt;_COS_NNN</code></td><td>CharacterEditor.rdb</td></tr>
      <tr><td>脸</td><td><code>&lt;角色&gt;_FACE_NNN</code></td><td>CharacterEditor.rdb</td></tr>
      <tr><td>头发</td><td><code>&lt;角色&gt;_HAIR_NNN</code></td><td>CharacterEditor.rdb</td></tr>
      <tr><td>贴图</td><td><code>MPR_Muscle_Character_&lt;角色&gt;&lt;COS&gt;&lt;NNN&gt;_…</code></td><td>MaterialEditor.rdb（45GB）</td></tr>
    </table>

    <h3>一键出带材质 blend（export_full.ps1）</h3>
    <pre>cd E:\\code\\othercode\\ripper_tpose\\scripts\\doa6

.\\export_full.ps1 KAS -Label KAS_Kasumi        # 默认 COS_001/HAIR_001/FACE_001
.\\export_full.ps1 MOM -Cos 102 -Label MOM_DLC  # 指定服装编号
.\\export_full.ps1 -List                        # 列出全部 1536 个模型</pre>
    <p>产物是 <code>_blends\\&lt;Label&gt;.blend</code>（贴图已打包）和同名
      <code>_preview.png</code>。</p>

    <h3>mod 变体（含 nude）</h3>
    <pre>.\\export_nude_mod.ps1 D:\\mods\\xxx.zip -Chr HEL -Label HEL_Helena_Nude
.\\export_nude_mod.ps1 D:\\mods\\ayane.zip -Chr AYA -Label AYA_Malf -Assign "3=f01,5=body"</pre>
    <p>吃 REDELBE layer2 mod（zip 或目录），自动接上该角色已导出的官方发型/脸。
      紫色徽标标出的就是这类变体。两条材质路线自动选择：</p>
    <table>
      <tr><th>路线</th><th>触发条件</th><th>可靠性</th></tr>
      <tr><td>A 精确</td><td>mod 替换的服装编号本机有（如 <code>HEL_COS_001</code>）→ 走原版 ktid 链</td><td>一次到位</td></tr>
      <tr><td>B 启发式</td><td>替换未安装的 DLC 位（如 <code>MOM_COS_105</code>）→ 按网格大小猜部位</td><td>多数一次对，弄反时用 <code>-Assign</code> 纠正</td></tr>
    </table>
    <div class="note"><b>官方内容没有 nude</b>（逐条目核实过）。mod 主要来源：
      GameBanana（game id <code>6966</code>，可脚本化直连下载，但多为微比基尼/走光级别）；
      全裸整合包在 LoversLab / DeviantArt，需登录手动下载。</div>

    <h3>已知坑</h3>
    <table>
      <tr><th>症状</th><th>原因 / 处理</th></tr>
      <tr><td>Noesis 打开就崩</td><td><b>别用 Cethleann.DataExporter 解 DOA6</b>：它的 zlib
        每块只读一次，1536 个 g1m 里 1300 个在 ~80% 处截断，坏文件让插件静默崩溃。
        用仓库的 <code>extract_rdb.py</code></td></tr>
      <tr><td>客串角色导出失败</td><td>NIC / MAI / SNK 的 <code>COS_000~003</code>
        是无 ktid 的占位调试体，正装从 <code>COS_004</code> 起</td></tr>
      <tr><td>Phase 4 头发是白模</td><td>她的 <code>HAIR_001</code> 在 MaterialEditor 里确实没有贴图</td></tr>
      <tr><td>mod 变体皮肤/衣服贴图弄反</td><td>路线 B 的启发式猜错，按脚本打印的
        <code>assign</code> 把两个部位对调后加 <code>-Assign</code> 重跑</td></tr>
      <tr><td>.ps1 报 missing terminator</td><td>脚本被存成无 BOM 的 UTF-8，
        PowerShell 5.1 按 GBK 解析吞掉引号；用 UTF-8 with BOM 重存</td></tr>
    </table>

    <h3>重新生成本页</h3>
    <pre>blender --background --factory-startup --python scripts\\doa6\\html\\collect_manifest.py
python scripts\\doa6\\html\\make_gallery.py</pre>
