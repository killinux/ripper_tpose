...
if len(part_objects) > 1:
    ...
    if body_bb:
        ...
        for pdir, objs in part_objects[1:]:
            ...
            # 是否已处在身体坐标系：看部件**顶端**是否落在身体上部 1/4 内。
            # 用顶端而不是底端——马尾/长发的底端会垂到腰部，用底端会误判；
            # 余量取身体高度 25%：翅膀（女天狗）、兔耳/帽子（DLC 兔女郎装）会把身体
            # 包围盒顶端撑高 10~20%，余量 5% 时脸和头发会被误判成未就位而整体上移。
            # 真正未就位的头发（用自己原点、悬在腰腹）顶端只到 0.6 左右，仍会被判出来。
            in_place = p_mx.z >= b_mx.z - body_h * 0.25
            if in_place:
