...
{cards}  </div>
  ...
  <section class="appendix">
    <h2>附录 · 导出脚本用法</h2>
    <p>脚本在 <code>scripts\\stellarblade\\</code>。完整说明见 <code>docs\\stellar-blade-extraction.md</code>，
      服装编号→名称对照见 <code>docs\\stellar-blade-eve-outfits.md</code>。</p>

    <h3>一套服装一条命令</h3>
    <pre>cd E:\\code\\othercode\\ripper_tpose\\scripts\\stellarblade
.\\export_eve.ps1                       # 先跑一次：标准 Eve + 共享的脸 / 发型 / 马尾 / UEFormat 快照
.\\export_outfit.ps1 CH_P_EVE_45_TypeB  # 任意服装：UE Viewer 按包名导 PSK+PNG -> Blender 组装 -> 渲染 + 报告
python list_models.py --glob 'CH_P_EVE_*'   # 从 .utoc 索引枚举包名（DLC 加 --path-filter SB/Content/DLC）</pre>
    <p>产物 <code>blender\\Eve_&lt;包名&gt;.blend</code>（单一主骨骼 <code>Eve_Armature</code>，
      Face_003 带 53 个表情 Morph，贴图外链）、<code>validation\\Eve_&lt;包名&gt;.png / _face.png / .json</code>。
      画廊用的亮预览由 <code>scripts\\final\\html\\render_blend_preview.py -- blender --suffix _gallery</code> 补渲。</p>

    <h3>包名规律</h3>
    <table>
      <tr><th>形式</th><th>含义</th></tr>
      <tr><td><code>CH_P_EVE_NN</code> / <code>NN_Body</code></td><td>本体服装（02–63，12/13/38/44 不存在）</td></tr>
      <tr><td><code>_TypeB</code> / <code>_TypeC</code> / <code>_Body_02</code></td><td>换色变体（名称见对照表）</td></tr>
      <tr><td><code>NH</code></td><td>无高跟版本</td></tr>
      <tr><td><code>Nier_01~04</code> / <code>Nikke_01~06</code></td><td>NieR / NIKKE 联动 DLC，挂在 <code>SB/Content/DLC_1|2/</code></td></tr>
      <tr><td><code>InnerSuit</code></td><td>Skin Suit；EveOriginalProportions 裸模 mod 覆盖的就是它</td></tr>
    </table>
    <div class="note"><b>原版没有 nude</b>。裸模来自本机安装的 EveOriginalProportions mod（<code>Eve_Nude_*.blend</code>）。</div>

    <h3>已知坑</h3>
    <table>
      <tr><th>症状</th><th>原因 / 处理</th></tr>
      <tr><td>UE Viewer 报 Found N game files 数量不对</td><td>用错构建；必须用专用的 <code>umodel_stellar_blade_v6.exe</code></td></tr>
      <tr><td>Face_003 导不进 Blender 3.6</td><td>UEFormat 导入器要用钉住的快照 + 仓库补丁（export_eve.ps1 自动处理）</td></tr>
      <tr><td>裸模发型悬空</td><td>mod 骨架缺 <code>SC_Hair</code>，用 <code>--alignment-reference</code> 复用标准报告的变换</td></tr>
    </table>

    <h3>重新生成本页</h3>
    <pre>python scripts\\stellarblade\\html\\collect_manifest.py
python scripts\\stellarblade\\html\\make_gallery.py</pre>
