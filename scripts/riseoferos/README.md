...
>
> - **辅助骨挂错父级**（`reparent_joint_helpers`，两次用户报告同一根因）：ROE 的肢体辅助骨挂在哪根骨上比较随意，
>   但它们承担大量蒙皮，于是肢体一动、这部分皮肤留在原地。
...
>
>   判据要两个条件同时成立：骨名符合 Biped 的肢体辅助骨命名（`twist|elbow|knee|ankle|muscle strand`，
>   是通用约定不是逐模型硬编码），**且**它的蒙皮重心投影落在某个肢段上。对每个肢段按
>   「横向距离 + 超出骨段的惩罚 + 偏向远端骨」打分取最优，且只有当最优肢段不是它当前所属时才重挂。
...
> - **骨名不统一**：见上面第 2 步。
>
> manifest 的 `mmdConvert` 记录骨数、Biped 前缀、手臂角、权重孔洞、重挂的辅助骨、撕裂统计、
> 刚体/关节数、表情列表和没匹配上的骨槽。
...
**判据**：重挂父级本身是安全的（保持 head/tail/roll，休息姿势下网格不动），但挂错骨就不安全了——
`butt_*`、`skirt_*`、`Pauldrons_*` 同样紧贴大腿根和肩部，挂到肢体上会让裙子跟着腿甩。所以要两个信号同时成立：
...
2. **蒙皮**：把该骨驱动的顶点算加权重心，投影到每条肢段（肩→肘、肘→腕、胯→膝、膝→踝），
   ...
   偏向远端是为了让正好骑在关节上的护垫跟随会转动的那一节。

只有当胜出的肢段**不是**它当前所属的那条时才重挂；父级已经要被移动的辅助骨会跟着父级走，不重复处理。
已经转换成 MMD 的模型另外识别 D 骨链（`足D`/`ひざD`），挂在 D 骨下属于正常，不会误报。

输出表逐骨列出：当前父级、蒙皮实际属于哪条肢段、投影位置 t、横向偏移比例和判定
（`misparented` / `ok` / `ok (D chain)` / `carried by its parent` / `no skin`）。
