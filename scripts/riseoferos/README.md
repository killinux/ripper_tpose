...
> **PMX 走 Convert_to_MMD5 转换**（`E:\code\othercode\Convert_to_MMD5`，Blender 3.6 插件，需与 mmd_tools
...
>    仍缺的关节再用「下一节骨的父级」补。盆骨映射到 下半身 保住臀部权重，センター 留空由插件重建。
> 3. **A-pose**：上臂绕世界 Y 轴放到 MMD 标准的 37°，用插件的 `_bake_pose_delta_to_rest` 连网格一起
>    烘成新休息姿势。VMD 的旋转是相对休息姿势的，T-pose 直接导会让手臂整体高 37°。
> 4. **一键转换 + 补齐 + 物理**：改名、补 センター/腰/腰キャンセル/上半身1、足ＩＫ/つま先ＩＫ、腿 D 骨、
...
>   - *手腕被撕开*：前臂扭转骨 `ForeTwist` 挂在**上臂**下、是前臂的兄弟骨，承担的手腕蒙皮比手骨还多
>     （男性裸模 309 顶点/权重 246，手骨才 193）。插件第 1.5 步拉直前臂时它不跟着转，手腕就断开。
>   - *膝盖有断层*：大腿扭转骨 `ThighTwist` 挂在**脊椎**下，带着从胯到膝盖上方的皮肤（j10 是 393 顶点/权重 229）。
>     动作时膝盖以上跟躯干、以下跟小腿，膝盖处出现一圈接缝。**静止时看不出来，撕裂门禁抓不到，只有摆姿势渲染才能发现。**
>   - *膝盖鼓包*：把它们改挂到远端骨之后，这块刚性的皮肤孤岛吃满了整个关节的旋转，
>     而周围平滑混合的皮肤只吃一半，孤岛被剪出表面。`knee_L` 的 994 个顶点从膝上 t=-0.21 铺到膝下 t=+0.26，
>     跟大腿会撕、跟小腿会鼓。
>
>   所以不是二选一，而是量：骨名要符合 Biped 的肢体辅助骨命名（`twist|elbow|knee|ankle|muscle strand`，
>   是通用约定不是逐模型硬编码），蒙皮重心要投影在某个肢段上（对每段按「横向距离 + 超出骨段的惩罚 +
>   偏向远端骨」打分取最优），**再**把它的每个顶点投影到远端骨上算跟随比例：越过关节 0.35 个骨长完全跟随，
>   反向 0.35 个骨长完全不跟随，按权重取平均。转换前统一挂到远端骨（这样插件掰直前臂、A-pose 压手臂时
>   它们都跟着走，休息姿势不会撕），转换后再把比例不足 97% 的往上提一级、加一条 MMD 回転付与。
>   a10 实测：`knee_L` 52%、`muscle_elbow` 46%、`UpArmTwist` 63%、`CalfTwist` 79%、`ForeTwist` 82%、
...
>   拉成一根长刺伸向手肘。判据卡得很窄：插件**新建**的骨拿到权重是正常的（`足D` 链、`上半身2`、
>   `足先EX` 就是干这个的），本来就有蒙皮的骨拿到更多也正常；只有**源骨架里就存在、原本一点蒙皮
>   都没有、转换后却有了**的骨才是被硬塞的，把那些顶点的原始权重还回去。120 个里有 7 个触发，
>   收权重的清一色是配饰/约束骨：a10 还回 467 个顶点（`AC_suspenders_*`、`AG_Spine1_*_tag`、
>   `AG_buckle_LB`、`Point_elbow_R`），e07 还回 383 个（其中 381 个是身体皮肤被焊到了枪套
>   `AC_holster_L` 上），f06 48 个（`Clock_constraint`），f05 与 f05_outfit1 各 10 个（`AC Head`），
>   a07 6 个（`AC collar_L/R`），c05 2 个（`AC cheek_bone_*`）。
>   顶点自己的骨要是被插件退役了就不动它——那种情况下转移是唯一出路，e07 的枪套网格就是这样从
>   `holster_L` 挪到父骨 `AC_holster_L` 上的，属于正常合并，同一根骨上那 381 个身体顶点才是要还回去的。
>   只还权重不动几何：塞错的权重在插件自己烘姿势时就已经生效，那 11 个顶点在休息姿势里也偏了两厘米，
>   撕裂门禁报的 4 条边就是这个残留（试过按骨骼位移反推顶点位置，结果 4 条变 30 条，作罢）。
> - **付与的变形顺序**（`verify_grant_order`）：MMD 按「变形阶层 + 骨序号」依次变形，
>   付与只有在**来源骨先变形**时才生效。这一条在 Blender 里完全看不出来——mmd_tools 用约束实现付与，
>   约束按依赖关系求值，跟骨序号无关。实测：`両目` 是 worker 追加的，排在骨列表末尾（如 m02 是 128 号），
>   而它驱动的 `左目`/`右目` 是 86/87 号，于是在 MMD 里 VMD 的视线关键帧根本不起作用。
...
>
>   **剩下 6 个告警逐个查清了**（用 `locate_tears2.py` 的做法：跑完整流程后按门禁自己的判定
>   逐条列出边，报出两端各由哪根骨驱动、属于哪个材质、在身高百分之几处）：
>
>   | 模型 | 边数 | 位置 | 两端骨 | 结论 |
>   |---|---|---|---|---|
>   | i04 | 49 | 身高 45%，身前 | `AC_handcuff_L/R` ↔ `上半身2` | 双手被手铐链在一起，A-pose 把两只手分开，链条那圈布只能拉长。**模型自带**，任何动作都会拉 |
>   | e10 | 8 | 81~82% 肩部 | `shoulder_L/R` ↔ `左肩/右肩` | 服装肩甲骨不跟手臂走，手臂下放 37° 时肩缝张开 |
>   | g11 | 7 | 85% 领口 | `collar_L/R_01` ↔ `UpArmTwist` | 同上，是领子与肩的接缝 |
>   | e03 | 2 | 75% 腋下 | `LUpArmTwist` ↔ `上半身3` | 同上，腋下 |
>   | a05 | 1 | 85% 肩 | `左肩` ↔ `左肩`（次权重不同） | 同上 |
>   | a10 | 4 | 70% 右肘 | `Point_elbow_R` ↔ `右ひじ` | **不是接缝**，是上面那条乱转权重的残留 |
>
>   前五个都是同一件事：**手臂下放 37° 时，服装的肩/领骨留在原地**。这是 A-pose 的必要代价——
>   不下放手臂，VMD 的旋转就整体偏 37°，模型全程举着手跳舞。已经用 `relax_shoulder_weights`
>   软化过一次；再软化会把硬挺的领子和肩甲揉成一团，不值当。**逐个渲图看过，接受。**
> - **肩部松弛**：手臂下放 13~47° 会在躯干与手臂权重的硬边界剪切皮肤，烘焙前先轻度平滑肩/腕/ひじ 权重。
> - **骨名不统一**：见上面第 2 步。
>
> manifest 的 `mmdConvert` 记录骨数、Biped 前缀、手臂角、权重孔洞、重挂的辅助骨（`reparentedHelpers`）
> 与它们的付与比例（`helper_grants`）、隐藏的透明材质（`hidden_materials`）、
> 撤销的乱转权重（`stray_weights`）、付与顺序违例（`grant_order_violations`）、撕裂统计、
> 刚体/关节数、表情列表和没匹配上的骨槽。
>
> **验收要看新渲的视频**：`<id>\blend\pmx\<stem>_dance.mp4` 是单独一步渲出来的，改了 worker 之后
> 不重渲就还是旧模型。a10/m02 报上来的「大腿根和膝盖有问题」就是这么来的——PMX 里
> `ThighTwist` 已经挂到 `足D` 了，视频里还挂在 `上半身`。要确认视频对应的是哪一版，
> 打开 `_dance.blend` 看 `Bip001 LThighTwist` 的父级即可。
> 仍未做：游戏自带扭转辅助骨保留但无 VMD 驱动；裙骨等英文名骨会留在显示枠里。眉毛/舌头/嘴型等表情 2026-09-19 起由 mmd_face_morphs 插件补齐（manifest 里 `face_morphs` 列表、`face_style` 拼法）。
> 男性裸模 a00 / a03–a06 没有可量尺寸的面部骨（只有牙齿和 Xtra 骨），插件拒绝在猜的尺寸上建，表情数为 0（属正常）。
...
缺贴图和被二次解析补挂的槽各有一个角标；有 XPS/PMX 的多一行路径，PMX 那行还带
骨数/刚体/关节/表情数。**有跳舞预览的直接内嵌一个播放器**（`preload="none"`，不点不加载），
「跳舞」那一行链的是 mp4 本身而不是所在目录；预览比 PMX 旧会标「已过期」。
附录是导出脚本用法；末尾列出 17 个 NOMESH 的 ID。页面用 `file://` 链接本机文件，**缩略图写到
...
## 5.5 fix_limb_helpers.py —— 肢体辅助骨挂错父级的诊断与修复

**什么时候用**：模型跳舞时某个关节出现「断层」——皮肤在膝盖、手腕、肘部裂开或错位，
但静止站着看不出来。PMX 导出流程里已经自动做了这件事，这个脚本是给**其它模型/其它来源**
用的独立工具，也可以用来复查。
...
# 修复并另存
blender --background --python fix_limb_helpers.py -- --input model.blend --fix --save model_fixed.blend

# 修复并覆盖原文件
blender --background --python fix_limb_helpers.py -- --input model.blend --fix --save -
```

输入可以是 `.blend`，也可以是 `.pmx`（用 mmd_tools 导入；`--save` 只写 .blend，PMX 要从中重新导出）。
关节骨优先用 ROE 的 Biped 解析器识别，识别不出来就退回 MMD 骨名，所以转换前后都能跑。

**原理**：游戏骨架里有一批「辅助骨」——扭转骨（`ForeTwist`/`ThighTwist`/`CalfTwist`/`UpArmTwist`）
和关节垫（`Point_elbow`/`AC elbow`/`knee_L`/`muscle_elbow`）。它们不是摆设，往往承担的蒙皮比关节骨本身还多
（ROE 男性裸模的 `ForeTwist` 在手腕有 309 顶点/权重 246，手骨才 193）。
但游戏把它们挂在当时顺手的骨上，靠运行时的约束纠正，而这些约束导不出来：

- `ForeTwist` 挂在**上臂**下，是前臂的兄弟骨；
- `ThighTwist` 挂在**脊椎**下，是大腿的兄弟骨。

皮肤却仍然属于前臂和大腿。于是肢体一动、辅助骨不动，同一块表面就被撕成两半：
拉直肘部进入 MMD 休息姿势时手腕裂开；跳舞时大腿皮肤跟着躯干、小腿跟着腿，膝盖处出现一圈接缝。
**膝盖这种只在动作中出现，静止时看不见，导出流程的几何门禁抓不到，必须摆姿势渲染才能发现。**

**只重挂父级还不够**。辅助骨是一整块刚性的「皮肤孤岛」，周围的皮肤是平滑混合的。
把它直接挂到远端骨上，它就吃满了整个关节的旋转，而旁边的皮肤只吃一半——孤岛被剪出表面，
膝盖上鼓出一个肉瘤。`knee_L` 最典型：它的 994 个顶点从膝关节上方 t=-0.21 一直铺到下方 t=+0.26，
跟着大腿（0%）会撕，跟着小腿（100%）会鼓，**只有一半才像个膝盖**。

**判据**：所以不是分类，而是量。三步：

1. **名字**符合 3ds Max Biped 的辅助骨命名（`twist`/`elbow`/`knee`/`ankle`/`muscle strand`），
   或者它的头部正好落在某个关节上。这是骨架通用约定，不是针对单个模型的硬编码。
   这一条挡住了 `butt_*`、`skirt_*`、`Pauldrons_*`——它们同样紧贴大腿根和肩部，
   挂到肢体上会让裙子跟着腿甩。
2. **蒙皮**：把该骨驱动的顶点算加权重心，投影到每条肢段（肩→肘、肘→腕、胯→膝、膝→踝），
   按「横向距离 + 超出骨段的惩罚 + 偏向远端骨的小偏置」给每条肢段打分，取最优。
3. **跟随比例**：把它的每个顶点投影到远端骨上，越过关节一个混合宽度（`GRANT_BLEND`，
   骨长的 0.35）就完全跟随，反向一个混合宽度就完全不跟随，按权重取平均。
   骑在关节上的护垫落在 0.5 左右，肢体中段的扭转骨落在 1.0 附近。

导出时先把所有辅助骨**统一挂到远端骨**——插件会掰直前臂、导出流程会把手臂压成 A-pose，
掉队的骨会在休息姿势里撕开皮肤，那是之后再也补不回来的。等转换完成、腿长出 D 骨链之后，
再把「只该跟一部分」的骨往上提一级（挂到关节的近端骨），并给它一条 MMD 的
**回転付与**（付与），比例就是上面算出来的数。重挂父级保持 head/tail/roll，休息姿势下网格不动，
变的只有动画。付与的来源取远端骨的 FK 骨（`ひざ` 而不是 `ひざD`），避免 PMX 里出现付与套付与。

a10 实测：`knee_L` 52%、`muscle_elbow` 46%、`UpArmTwist` 63%、`CalfTwist` 79%、
`ForeTwist` 82%、`ThighTwist` 89%，末端的 `*Twist1` 都是 99–100%。

已经转换成 MMD 的模型另外识别 D 骨链（`足D`/`ひざD`），挂在 D 骨下属于正常，不会误报。

输出表逐骨列出：当前父级、蒙皮实际属于哪条肢段、投影位置 t、横向偏移比例、跟随比例和判定
（`misparented` = 现在的父级带不动它的皮肤 / `over-following` = 跟得太满 / `ok` /
`ok (twist chain)` / `no skin`）。已经修好的模型会读出「想要 79%、实际 79%」这样的一行，
所以对转换前后的骨架都能用：刚性子骨（含挂在 `足D`/`ひざD` 上的）算 100%，被提上去的算它的付与比例。

---


## 5.6 render_pmx_dance.py / .ps1 —— PMX 跳舞预览

骨骼问题只有动起来才看得见，所以每个 PMX 旁边放一段用真实 VMD 渲的 mp4。
...
PMX 里 `ThighTwist` 已经挂到 `足D`，视频里还挂在 `上半身`。改完 worker 一定要重渲。

> 这个 ps1 带 UTF-8 BOM：默认 VMD 路径里有中文，PowerShell 5.1 读无 BOM 的脚本会按
> ANSI 解码，路径直接变乱码。目录里其它 ps1 是纯 ASCII，所以不需要。
