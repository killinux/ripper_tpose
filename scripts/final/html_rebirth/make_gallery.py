...
def collect(manifest_path, thumb_dir, force):
    ...
    for entry in manifest.get("results", []):
        ...
        models.append({
            ...
            "route": entry.get("route") or "FModel",
...
def render_card(model):
    ...
    if model["route"] == "CUE4Parse CLI":
        badges += '<span class="badge badge-code" title="FModel 导不出，ff7rb_cli_export.py 补导">CLI 补导</span>'
    mod = model["mod"]
    mod_row = ""
    ...
    export_links = " · ".join('<a href="%s">%s</a>' % (esc(file_uri(exports[k])), label)
                              ...
                              if exports.get(k))
    if mod:
        mod_row = ('<dt>mod</dt><dd><a href="%s" target="_blank" rel="noopener">%s</a> · %s%s</dd>'
                   % (esc(mod.get("url", "")), esc(mod.get("name", "")), esc(mod.get("file", "")),
...
def render(models, source_root):
    ...
    kinds = [k for k in ("official", "cutscene", "toad", "variant", "mod") if any(m["kind"] == k for m in models)]
    per_char = {}
    for m in models:
